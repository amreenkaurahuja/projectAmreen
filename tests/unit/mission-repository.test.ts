...
describe("mission repository: getActiveQuestionsBySubject", () => {
  it("returns active questions for the subject", async () => {
    const supabase = {
      from: vi.fn((table: string) => {
        if (table === "subjects") {
          return {
    ...
    const repository = new SupabaseMissionRepository(supabase as never);
    const questions = await repository.getActiveQuestionsBySubject(
      "mathematics",
      8,
    );

    expect(questions).toEqual([
      { id: "question-1", subjectSlug: "mathematics" },
    ]);
  });

  it("throws MissionQuestionBankError when the subject does not exist", async () => {
    const supabase = {
      from: vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
    ...
    await expect(
      repository.getActiveQuestionsBySubject("mathematics", 8),
    ).rejects.toThrow(MissionQuestionBankError);
  });

  it("throws MissionQuestionBankError when the subject lookup fails", async () => {
    const supabase = {
      from: vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
  ...
  it("throws MissionQuestionBankError when the question query fails", async () => {
    const supabase = {
      from: vi.fn((table: string) => {
        if (table === "subjects") {
          return {
